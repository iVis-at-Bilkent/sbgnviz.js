...
module.exports = function () {
  ...
  graphUtilities.updateGraph = function (
    cyGraph,
    callback,
    layoutOptions,
    tileInfoBoxes
    ...
    cy.nodes().forEach(function (node) {
      ...
      for (var j = 0; j < statesandinfos.length; j++) {
        classes
          .getAuxUnitClass(statesandinfos[j])
          .setParentRef(statesandinfos[j], node);
      }
    });
      ...
      cy.nodes().forEach(function (node) {
        if (elementUtilities.isPNClass(node) && node.parent().length == 0) {
          ...
          node
            ...
            .forEach(function (neighbor) {
              if (processParentMap.has(neighbor.parent().id()))
                processParentMap.set(
                  neighbor.parent().id(),
                  processParentMap.get(neighbor.parent().id()) + 1
                );
              else processParentMap.set(neighbor.parent().id(), 1);
            });
          ...
          processParentMap.forEach((value, key) => {
            if (
              max_count < value ||
              (max_count == value && result == undefined)
            ) {
              result = key;
              max_count = value;
            }
    ...
    $(document).trigger("updateGraphEnd", [
      cy,
      isLayoutRequired || tileInfoBoxes,
      performLayout,
    ]);
  ...
  graphUtilities.calculatePaddings = function (paddingPercent) {
    ...
    if (!paddingPercent) {
      ...
      paddingPercent =
        typeof compoundPadding === "function"
          ? compoundPadding.call()
          : compoundPadding;
    }
    ...
    var calc_padding =
      (paddingPercent / 100) * Math.floor(total / (2 * numOfSimples));
    if (calc_padding < 5) {
      calc_padding = 5;
    }
  ...
  graphUtilities.recalculatePaddings = graphUtilities.refreshPaddings =
    function () {
      ...
      var compoundPadding = options.compoundPadding;
      return typeof compoundPadding === "function"
        ? compoundPadding.call()
        : compoundPadding;
