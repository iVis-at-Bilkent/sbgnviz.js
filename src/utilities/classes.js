...
var AuxiliaryUnit = function (parent) {
    if (parent) {
        this.parent = typeof parent === 'string' ? parent : parent.id(); // Keep if of parent to avoid circular references
    }
...
AuxiliaryUnit.prototype.getParentNode = function() {
...
AuxiliaryUnit.prototype.updateParent = function(parent){
    this.parent = parent;
};
...
AuxiliaryUnit.prototype.copy = function (existingInstance, newParent, newId) {
    ...
    newUnit.parent = newParent ? newParent : this.getParentNode();
...
AuxiliaryUnit.prototype.drawText = function(context, centerX, centerY) {
    ...
    context.globalAlpha = this.getParentNode().css('text-opacity') * this.getParentNode().css('opacity'); // ?
...
AuxiliaryUnit.prototype.getAbsoluteCoord = function() {
    if(this.coordType == "relativeToCenter") {
        var absX = this.bbox.x * (this.getParentNode().outerWidth() - this.getParentNode()._private.data['border-width']) / 100 + this.getParentNode()._private.position.x;
        var absY = this.bbox.y * (this.getParentNode().outerHeight() - this.getParentNode()._private.data['border-width']) / 100 + this.getParentNode()._private.position.y;
        return {x: absX, y: absY};
    }
    else if(this.coordType == "relativeToSide") {
        if (this.anchorSide == "top" || this.anchorSide == "bottom") {
            var absX = this.getParentNode()._private.position.x - (this.getParentNode().outerWidth() - this.getParentNode()._private.data['border-width']) / 2 + this.bbox.x;
            var absY = this.bbox.y * (this.getParentNode().outerHeight() - this.getParentNode()._private.data['border-width']) / 100 + this.getParentNode()._private.position.y;
        }
        else {
            var absY = this.getParentNode()._private.position.y - (this.getParentNode().outerHeight() - this.getParentNode()._private.data['border-width']) / 2 + this.bbox.y;
            var absX = this.bbox.x * (this.getParentNode().outerWidth() - this.getParentNode()._private.data['border-width']) / 100 + this.getParentNode()._private.position.x;
        }

        // due to corner of barrel shaped compartment shift absX to right
        if (this.getParentNode().data("class") == "compartment"){
            absX += this.getParentNode().outerWidth() * 0.1;
        };
        return {x: absX, y: absY};
    }
};
...
AuxiliaryUnit.prototype.addToParent = function (parentNode, location, position, index) {
    ...
    if(typeof index != "undefined") { // specific index provided (for undo/redo consistency)
        self.getParentNode().data('statesandinfos').splice(index, 0, this);
    }
    else {
        self.getParentNode().data('statesandinfos').push(this);
    }

    if(!self.getParentNode().data('auxunitlayouts')) { // ensure minimal initialization
        self.getParentNode().data('auxunitlayouts', {});
    }
    ...
    if(!self.getParentNode().data('auxunitlayouts')[location]) {
        self.getParentNode().data('auxunitlayouts')[location] = new ns.AuxUnitLayout(parentNode, location);
    }
    var layout = self.getParentNode().data('auxunitlayouts')[location];
...
AuxiliaryUnit.prototype.removeFromParent = function () {
    var self = this;
    var parentLayout = this.getParentNode().data('auxunitlayouts')[this.anchorSide];
    parentLayout.removeAuxUnit(this);
    if (parentLayout.isEmpty()){
        delete self.getParentNode().data('auxunitlayouts')[this.anchorSide];
    }
    var statesandinfos = self.getParentNode().data('statesandinfos');
    var index  = statesandinfos.indexOf(this);
    statesandinfos.splice(index, 1);
};

AuxiliaryUnit.prototype.getPositionIndex = function() {
    return this.getParentNode().data('auxunitlayouts')[this.anchorSide].units.indexOf(this);
};
...
StateVariable.create = function(parentNode, value, variable, bbox, location, position, index) {
    ...
    return {
        index: stateVar.getParentNode().data('statesandinfos').indexOf(stateVar),
...
StateVariable.prototype.remove = function () {
    ...
    var index = this.getParentNode().data('statesandinfos').indexOf(this);
...
UnitOfInformation.create = function (parentNode, value, bbox, location, position, index, shapeFn, shapeArgsFn) {
    ...
    if (parentNode) {
        // Use id of parent node instead of the node itself to avaoid circular references
        unit.parent = typeof parentNode === 'string' ? parentNode : parentNode.id();
    }
    unit.bbox = bbox;
    ...
    return {
        index: unit.getParentNode().data('statesandinfos').indexOf(unit),
...
EntityType.prototype.createNewDefinitionFor = function (stateVar) {
    ...
    stateVar.getParentNode().data('entityType', this);
...
var AuxUnitLayout = function (parentNode, location, alignment) {
    ...
    if (parentNode) {
        this.parentNode = typeof parentNode === 'string' ? parentNode : parentNode.id(); // Keep id of parent node to avaoid circular references
    }
    ...
    if(this.getParentNode().data('class') == "simple chemical") {
        this.outerMargin = 3;
    }
};

AuxUnitLayout.prototype.getParentNode = function() {
...
AuxUnitLayout.prototype.getDrawableUnitAmount = function() {
    ...
    var availableSpace;
    if (this.isTorB()) {
        availableSpace = self.getParentNode().outerWidth();
    }
    else {
        availableSpace = self.getParentNode().outerHeight();
    }
...
AuxUnitLayout.prototype.setDisplayedUnits = function () {
    ...
    var availableSpace;
    if (this.isTorB()) {
        availableSpace = this.getParentNode().outerWidth();
        // due to corner of barrel shaped compartment decrease availableSpace -- no infobox on corners
        if (self.getParentNode().data("class") == "compartment")
            availableSpace *= 0.8;
    }
    else {
        availableSpace = self.getParentNode().outerHeight();
...
AuxUnitLayout.prototype.resizeParent = function (length) {
    if(this.isTorB()) {
        if(this.getParentNode().data('bbox').w < length) {
            cy.trigger("noderesize.resizestart", ["centerright", this.getParentNode()]);
            this.getParentNode().data('bbox').w = length;
            cy.trigger("noderesize.resizeend", ["centerright", this.getParentNode()]);
        }
    }
    else {
        if(this.getParentNode().data('bbox').h < length) {
            cy.trigger("noderesize.resizestart", ["bottomcenter", this.getParentNode()]);
            this.getParentNode().data('bbox').h = length;
            cy.trigger("noderesize.resizeend", ["bottomcenter", this.getParentNode()]);
        }
    }
};
...
AuxUnitLayout.prototype.setParentMinLength = function () {
    var parentLayouts = this.getParentNode().data('auxunitlayouts');
    ...
    if(this.isTorB()) {
        this.getParentNode().data('resizeMinWidth', Math.max(this.lengthUsed, compareVal));
    }
    else {
        this.getParentNode().data('resizeMinHeight', Math.max(this.lengthUsed, compareVal));
    }
};
