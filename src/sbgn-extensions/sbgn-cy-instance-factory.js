...
module.exports = function () {
  ...
  var sbgnCyInstance = function (param) {
     ...
     */
    var generateCircleString = function (
      centerX,
      centerY,
      radius,
      angleFrom,
    ...
    var generateShapeWithPortString = function (
      lineHW,
      shapeHW,
      type,
      orientation
      ...
      if (orientation === "horizontal") {
        ...
        if (type === "circle") {
          abovePoints = generateCircleString(
            0,
            0,
            shapeHW,
            180,
          ...
          belowPoints = generateCircleString(
            0,
            0,
            shapeHW,
            360,
          ...
          abovePoints =
            ...
            " ";
          belowPoints =
            shapeHW + " " + shapeHW + " -" + shapeHW + " " + shapeHW + " ";
        }
        ...
        if (type === "circle") {
          leftPoints = generateCircleString(
            0,
            0,
            shapeHW,
            90,
          ...
          rightPoints = generateCircleString(
            0,
            0,
            shapeHW,
            -90,
    ...
    function registerUndoRedoActions() {
      ...
      ur.action(
        "deleteElesSimple",
        undoRedoActionFunctions.deleteElesSimple,
        undoRedoActionFunctions.restoreEles
      );
      ur.action(
        "deleteNodesSmart",
        undoRedoActionFunctions.deleteNodesSmart,
        undoRedoActionFunctions.restoreEles
      );
      ur.action(
        "setPortsOrdering",
        undoRedoActionFunctions.setPortsOrdering,
        undoRedoActionFunctions.setPortsOrdering
      );
      ...
      ur.action(
        "removeAll",
        undoRedoActionFunctions.removeAll,
        undoRedoActionFunctions.restoreAll
      );
      ur.action(
        "hideExperiment",
        undoRedoActionFunctions.hideExp,
        undoRedoActionFunctions.unhideExp
      );
      ur.action(
        "unhideExperiment",
        undoRedoActionFunctions.unhideExp,
      ...
      ur.action(
        "hideFile",
        undoRedoActionFunctions.hideFile,
        undoRedoActionFunctions.hideFileUndo
      );
      ur.action(
        "unhideFile",
        undoRedoActionFunctions.unhideFile,
        undoRedoActionFunctions.unhideFileUndo
      );
      ur.action(
        "removeExperiment",
        undoRedoActionFunctions.removeExp,
        undoRedoActionFunctions.addExp
      );
      ur.action(
        "removeFile",
        undoRedoActionFunctions.removeFile,
        undoRedoActionFunctions.addFile
      );
      ur.action(
        "expButtonChange",
        undoRedoActionFunctions.expButtonChange,
        undoRedoActionFunctions.expButtonChange
      );
      ur.action(
        "fileButtonChangeHide",
        undoRedoActionFunctions.fileButtonChangeHide,
        undoRedoActionFunctions.fileButtonChangeUnHide
      );
      ur.action(
        "fileButtonChangeUnHide",
        undoRedoActionFunctions.fileButtonChangeUnHide,
        undoRedoActionFunctions.fileButtonChangeHide
      );
    ...
    function showTooltip(event) {
      ...
      var canHaveTooltip = function (node) {
        return (
          elementUtilities.isSIFNode(node) || node.data("tooltip") !== null
        );
      };
      ...
      if (!infobox) {
        ...
        modelPos.x -= modelW / 2;
        modelPos.y -= modelH / 2;
        var renderedPos = elementUtilities.convertToRenderedPosition(
          modelPos,
          pan,
          zoom
        );
    ...
    function bindCyEvents() {
      ...
      cy.on("beforeDo", function (e, name, args) {
        if (
          ...
          (name == "batch" &&
            ((args.length > 0 && args[0]["name"] == "thinBorder") ||
              (args.length > 0 && args[0]["name"] == "thickenBorder")))
        ) {
      ...
      $(document).on(
        "updateGraphEnd",
        function (event, _cy, isLayoutRequired, callback) {
          // if the event is not triggered for this cy instance return directly
          if (_cy != cy) {
            return;
          }
          var setCompoundInfoboxes = function (
            node,
            isLayoutRequired,
            cyInstance
          ) {
            ...
            for (var i = 0; i < node.data("statesandinfos").length; i++) {
              ...
              var infoBoxOnNode = classes.AuxiliaryUnit.setAnchorSide(
                statesandinfos,
                node
              );
            ...
            for (var i = 0; i < node.data("statesandinfos").length; i++) {
              ...
              if (
                (isLayoutRequired === undefined || !isLayoutRequired) &&
                correctInfoBoxCoord
              ) {
                var cordResult = classes.AuxiliaryUnit.convertToRelativeCoord(
                  statesandinfos,
                  bbox.x + bbox.w / 2,
                  bbox.y + bbox.h / 2,
                  cyInstance,
                ...
                if (!layouts[location]) {
                  ...
                  layouts[location] = classes.AuxUnitLayout.construct(
                    node,
                    location
                  );
                }
                // populate the layout of this side
                classes.AuxUnitLayout.addAuxUnit(
                  layouts[location],
                  cyInstance,
                  statesandinfos,
                  undefined,
                ...
                var location = classes.AuxUnitLayout.selectNextAvailable(
                  node,
                  cy
                );
                if (!node.data("auxunitlayouts")[location]) {
                  node.data("auxunitlayouts")[location] =
                    classes.AuxUnitLayout.construct(node, location);
                }
    ...
    var sbgnStyleSheet = cytoscape
      ...
      .css({
        "shape-polygon-points": function (ele) {
          if (
            ...
            return generateShapeWithPortString(
              0.01,
              shapeHW,
              type,
              orientation
      ...
      .css({
        padding: function () {
          return (
            graphUtilities.getCompoundPaddings() +
